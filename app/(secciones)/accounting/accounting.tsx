...
interface ContabilidadComponentProps {
    income: number;
    expenses: number;
    transactionsCustomer: ICustomerInvoices[];

}

const ContabilidadComponent: React.FC<ContabilidadComponentProps> = ({ income: incomeData, expenses: expensesData, transactionsCustomer: transactionsCustomerData }: ContabilidadComponentProps) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedFilter, setSelectedFilter] = useState('todos');
    const [income, setIncome] = useState(incomeData || 0);
    const [expenses, setExpenses] = useState(expensesData || 0);
    const [transactionsCustomer, setTransactionsCustomer] = useState<ICustomerInvoices[]>(transactionsCustomerData || []);
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        ...
                        <div className="flex space-x-2">
                            {['todos', 'ingresos', 'egresos'].map((filter, index) => (
                                <button
                                    key={index}
                                    onClick={() => setSelectedFilter(filter)}
                                    className={`px-4 py-2 rounded-lg font-medium transition-colors capitalize ${selectedFilter === filter
                                ...
                                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                                    <div className="flex-1">
                                        <div className="flex items-center space-x-4 mb-4 lg:mb-0">
                                            <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold ${transaction.type === 'Ingreso'
